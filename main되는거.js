...
var dlist = '';
var path = './data';
var p = fs.readdirSync(path, { withFileTypes: true });

while (i < p.length) {
  const dir = p[i].name
  if (p[i].isDirectory()) {  // 디렉토리인지 체크
      console.log(`${dir} is Directory!!!`);
      var test = `./${path}/${p[i].name}`; // 디렉토리이면 디렉토리의 파일 검색
      console.log(test);
      fs.readdir(test, function (error, filelist) {
          var j = 0;
          console.log(filelist);
          dlist = dlist + `<div class="accordion accordion-flush" id="accordionFlushExample">
          ...
          while (j < filelist.length) {
              dlist = dlist + `<li><a href="/?id=${filelist[j]}">${filelist[j]}</a></li>
              `;
              j = j + 1;
          }
          dlist = dlist + `</div >
